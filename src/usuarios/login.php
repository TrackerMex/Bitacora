...
function clear_login_output_buffer()
{
...
register_shutdown_function(function () {
    ...
    error_log("[usuarios/login] FATAL: " . ($error["message"] ?? "Error fatal"));
    if (!headers_sent()) {
        clear_login_output_buffer();
        http_response_code(500);
        header("Content-Type: application/json; charset=utf-8");
        echo json_encode(
            [
                "success" => false,
                "message" => "Error interno del servidor durante el login. Revisa la configuración y migraciones de producción.",
            ],
...
function fetch_user_tabs($conn, $usuario_id, $role)
{
    ...
    if (!$stmt) {
        throw new Exception(
            "Error preparando permisos de tabs: " . $conn->error,
        );
    }
    $stmt->bind_param("i", $usuario_id);
    if (!$stmt->execute()) {
        throw new Exception(
            "Error consultando permisos de tabs: " . $stmt->error,
        );
    }
    $res = $stmt->get_result();
...
function fetch_user_clients($conn, $usuario_id, $role)
{
    ...
    if (!$stmt) {
        throw new Exception(
            "Error preparando clientes de usuario: " . $conn->error,
        );
    }
    $stmt->bind_param("i", $usuario_id);
    if (!$stmt->execute()) {
        throw new Exception(
            "Error consultando clientes de usuario: " . $stmt->error,
        );
    }
    $res = $stmt->get_result();
    $clientes = [];
...
function fetch_allowed_units($conn, $usuario_id, $role, $clientes)
{
    ...
    $cliente_ids = array_values(
        array_filter(array_map(fn($c) => intval($c["id"] ?? 0), $clientes)),
    );
    ...
    if (!$stmt) {
        throw new Exception(
            "Error preparando unidades de cliente: " . $conn->error,
        );
    }
    $stmt->bind_param($types, ...$cliente_ids);
    if (!$stmt->execute()) {
        throw new Exception(
            "Error consultando unidades de cliente: " . $stmt->error,
        );
    }
    $res = $stmt->get_result();
    $unidades = [];
...
try {
    ...
    if ($role === "editor") {
        http_response_code(403);
        throw new Exception(
            "El rol editor no tiene acceso a la bitácora. Use un usuario lector.",
        );
    }
