/**
 * WEBHOOK PRODUCCIÓN - VERSIÓN FINAL CORRECTA
 *
 * ✅ Credenciales correctas:
 * - Datos están en: BITACORA_SHEET_ID (NO Planificador)
 * - Hoja se llama: "Datos" (NO "Tramos")
 * - Índices: unidad=2, tramo_id=20, actualizar=16-19
 */

const SHEETS_IDS = {
  BITACORA: "TU_BITACORA_SHEET_ID", // ✅ AQUÍ van los datos
};
...
function doPost(e) {
  try {
    ...
    const timestamp_recibida = Utilities.formatDate(
      new Date(),
      "GMT",
      "yyyy-MM-dd HH:mm:ss",
    );
...
/**
 * Busca en BITACORA_SHEET_ID > Hoja "Datos"
 * Columnas: 2=unidad, 20=tramo_id
 */
function buscarTramoActivo(unidad) {
  try {
    const ss = SpreadsheetApp.openById(SHEETS_IDS.BITACORA);
    const tramoSheet = ss.getSheetByName("Datos"); // ✅ HOJA "Datos"

    if (!tramoSheet) {
      throw new Error('Hoja "Datos" no existe en BITACORA_SHEET_ID');
    }
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const rowTramoId = data[i][20]; // Columna U
      const rowFolio = data[i][1]; // Columna B
...
/**
 * Actualiza columnas 16-19 (Q-T) en la hoja "Datos"
...
function actualizarTramo(unidad, columna, timestamp_evento) {
  try {
    ...
    const columnMap = {
      fecha_salida: 16, // Columna Q
      fecha_entrada_carga: 17, // Columna R
      fecha_salida_carga: 18, // Columna S
      fecha_descarga: 19, // Columna T
    };
    ...
    const row = [
      ...
      datos.timestamp_evento
        ? Utilities.formatDate(
            new Date(datos.timestamp_evento),
            "GMT",
            "yyyy-MM-dd HH:mm:ss",
          )
        : "",
...
function setupScriptProperties() {
  ...
  props.setProperty("BITACORA_SHEET_ID", "TU_BITACORA_ID_AQUI"); // ✅ ID de tu Sheets
...
function testWebhook() {
  ...
  const testPayload = {
    token: "orh5537498",
    unidad: "ALEXIS-01", // Cambiar por una unidad REAL de tu tabla
    event_type: "salida",
    pos_time: Utilities.formatDate(new Date(), "GMT", "yyyy-MM-dd HH:mm:ss"),
  ...
  try {
    ...
    const resultado = actualizarTramo(
      testPayload.unidad,
      "fecha_salida",
      testPayload.pos_time,
    );
